...
class Player(base_object.BaseObject):
    def __init__(self, graphics, manager, size, name=None):
        """
        Args:
    ...
    def draw(self):
        if not self.death:
            self.graphics.draw_line(self.position, utilities.get_move(self.position, self.rotation, 15), self.color)
            self.graphics.draw_circle(self.position, self.size, self.color)
            self.graphics.draw_circle(self.position, self.size + config.PLAYER_VISION, self.color, 1)
